...
import {FaSync, FaUserPlus} from 'react-icons/all'
import {APP_SECURITY_AUTHENTICATE_LOGIN_URL, APP_SECURITY_AUTHENTICATE_SIGNUP_URL, APP_ERROR_404_URL} from '../../../../url/AppUrl'
...
const SignUp = () => {
    ...
    const submitAction = () => {
        if (validateFieldsAction()) {
            setSuccess({message: `You have successfully sign up.`})
            setTimeout(() => {
                history.push(APP_SECURITY_AUTHENTICATE_LOGIN_URL)
            }, 3000)
        } else {
            setTimeout(() => {
                setErrorFields(null)
